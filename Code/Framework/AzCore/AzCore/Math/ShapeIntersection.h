...
namespace AZ
{
    ...
    {
        AZCORE_API bool IntersectThreePlanes(const Plane& p1, const Plane& p2, const Plane& p3, Vector3& outP);

        //! Tests to see which halfspace Arg2 is in relative to Arg1.
        AZCORE_API IntersectResult Classify(const Plane& plane, const Sphere& sphere);
        AZCORE_API IntersectResult Classify(const Plane& plane, const Obb& obb);

        //! Tests to see how Arg2 relates to frustum Arg1.
        AZCORE_API IntersectResult Classify(const Frustum& frustum, const Sphere& sphere);
